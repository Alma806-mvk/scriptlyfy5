...
export const lead = onRequest({ region: "us-central1" }, async (req, res) => {
    const ct = req.headers?.["content-type"] || undefined;
    console.log("[lead] request", { method: req.method, path: req.path || req.url, contentType: ct });
    ...
    try {
        ...
        const { type, email, role, useCase, challenge, count, company, meta, referralSource, } = (body ?? {});
